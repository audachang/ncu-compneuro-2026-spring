...
**檔案**：`demo/data/messy_stroop.csv` (n = 200 trials)

如果遺失，可在 `demo/data/` 執行 `python generate_messy_stroop.py` 重新生成（`seed=42`）。
...
- 規模小、跑得快
- 真實研究中常見的「髒」樣態都被刻意注入
- 內含一個可分析的 **Stroop effect** — cleaning 對了，你應該能在 incongruent 與 congruent 條件間看出 RT 差異
- **生成器原始碼公開**（`demo/data/generate_messy_stroop.py`）— 等於是這份資料的「最終 source of truth」，學生可以從它逆推欄位定義與注入的問題類型
...
打開 `demo/data/generate_messy_stroop.py`，閱讀並紀錄：
...
>
> 你會發現課堂 demo（`demo/03_pipeline/pipeline.py`）的 `clean()` 用了
...
- Sentinel 值**明確列出**（字串、整數都要寫）
- 三來源彼此**互相驗證**

一條會被扣分的條目：

```markdown
| `rt_ms` | float | 150–3000 ms | NA | 教師指定 |   ← ❌ 沒依據、抄作業說明
```

或：

```markdown
| `rt_ms` | float | 200–2500 ms | 99999 | Whelan (2008) |   ← ❌ 只抄文獻數字，沒解釋為何選這個、犧牲什麼
```

### 6.1 對 messy data 起手最常見的錯誤

❌ **無腦 `df.dropna()`**
```python
df = df.dropna()   # 不知道 drop 了什麼、drop 了多少
```
...
### 6.2 cleaning 與 analysis 的混淆

下面這行**不是** cleaning，是 analysis decision（會影響 condition mean）：

```python
df = df[df["rt_ms"] < df["rt_ms"].mean() + 3*df["rt_ms"].std()]
```

→ 應該放在 `analyse()` 並以 `outlier_sd=3.0` 形式暴露成參數。

### 6.3 cache 的常見錯誤

❌ **widget 寫在 cached function 內**
```python
@st.cache_data
def load_data():
    n = st.slider("rows", 10, 100)   # ← cache key 不含 slider 值！
    return pd.read_csv(...).head(n)
```
...
讀完 CSV 第一件事：`print(df.dtypes)`。如果 `rt_ms` 是 `object` 而不是 `float`，幾乎肯定混入字串（`"NA"`、空字串）。先 `pd.to_numeric(errors="coerce")`，才能跑 `.describe()`。
...
### 7.1 課堂材料
- 講義：`week-12-streamlit_caching_and_data_pipeline.md`
- 投影片：`week-12-slides.pptx`
- Demo 程式碼（**可參考但不要直接複製**）：
  - `demo/03_pipeline/pipeline.py` — 完整 `load_raw / describe / clean / analyse` 四層架構
  - `demo/00_week11_app/app_with_cache.py` — `@st.cache_data` 使用範例 + 頁內計時器
  - `demo/00_week11_app/app.py` — 七大區塊的繁體中文註解版
...
### 7.3 進階閱讀（可選）
- Wickham, H. (2014). *Tidy Data*. **Journal of Statistical Software**, 59(10). — 為什麼 cleaning 與 analysis 要分開
- Tukey, J. W. (1977). *Exploratory Data Analysis*. — descriptive statistics 作為「健康檢查」的奠基性論述
...
**選項 A — eeclass 上傳壓縮檔**
1. 把 `hw12_yourname/` 整個資料夾壓縮成 `hw12_yourname.zip`
2. 上傳至 eeclass 課程作業區

**選項 B — GitHub repo**（推薦，與 Week 10 git 教學銜接）
1. 在 GitHub 建立 public repo `ns5116-hw12-yourname`
2. push 完成後在 eeclass 繳交區貼 repo URL

⚠️ 不論哪種方式，請**確認 TA 可以直接看到** `report.ipynb` 的 output cells（不要交一個沒跑過的 notebook）。

---

## 9. 學術誠信 (Academic Integrity)

可以**討論觀念**、可以**互相 debug**、可以**參考 demo 程式碼學寫法**。

**禁止**直接複製其他同學的 `report.ipynb`、`app.py`、`clean()` 函式內容。每個學生提交的內容必須是自己理解後寫出來的。

**AI 工具（Claude Code / ChatGPT）使用規範**：
- ✅ 可以用 AI 幫你查 pandas API、debug error、解釋程式碼
- ⚠️ 若 AI 幫你產出超過 5 行的程式碼片段，請在 README 中**註明**哪幾段是 AI 協助、你做了什麼修改與驗證
- ❌ 禁止讓 AI 從頭寫完整份 `report.ipynb` 或 `app.py` — 這違反本課程強調的「親自走過 pipeline」目標

---

*最後更新：2026-05-14 · 張智宏 · ACL@NCU*
