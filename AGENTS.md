...
> **本文件目的**：指引 Codex 協助產生本課程的講義、範例程式碼、習題、與投影片。
...
| **Part 2: Vibe Coding & Agentic** | 9–16 | 用 AI 工具建構資料應用 | **Codex**, GitHub, **Streamlit**, pandas, requests, Plotly, **Anthropic SDK** |
...
- **Week 9**：Codex CLI、prompt engineering 基礎。
...
│           ├── creating_arrays.py
│           └── ...
├── homeworks/                       ← 學生作業與評分（見該資料夾的 AGENTS.md）
...
- **本檔案 (`/AGENTS.md`)**：教材撰寫指南。
- **`homeworks/AGENTS.md`**：作業下載、批改、上傳到 eeclass 的自動化流程（**不要**與本檔案混淆，那份檔案只用於 grading workflow）。

當任務涉及「produce / write / generate / 撰寫」教材時，請參考本檔案。
當任務涉及「grade / download submission / upload to eeclass」時，請參考 `homeworks/AGENTS.md`。
