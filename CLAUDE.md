...
| week07  | 67722      | Completed (scores uploaded; feedback attached as .md files, not PDFs) |
| midterm | 67993      | Completed (2 students: 呂杰驛=95, 何官臻=70; 吳心圓 did not submit) |
...
- **Automated grading timeout**: The bash sandbox has a 44-second max per call. For assignments requiring PDF upload, use `grade_midterm_noupload.py` pattern (skip upload if PDFs already in eeclass). Login takes ~13s, grading without upload ~12s/student.
- **Login**: Uses `/index/login` form POST (not AJAX). Session conflict modal ("登出所有裝置") must be handled by Playwright; requests-based login does NOT resolve the conflict automatically.
- **midterm-specific**: Only 呂杰驛 and 何官臻 submitted. Link text is `{name}_期中作品` not `{name}_Homework-week-NN`.
