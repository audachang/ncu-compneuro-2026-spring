...
# Read credentials from .eeclass_passwd (authoritative source)
_creds = {}
_passwd_file = SCRIPT_DIR / ".eeclass_passwd"
if _passwd_file.exists():
    for _line in _passwd_file.read_text(encoding="utf-8").splitlines():
        _line = _line.strip()
        if "=" in _line:
            _k, _v = _line.split("=", 1)
            _creds[_k] = _v

ACCOUNT = os.environ.get("EECLASS_ACCOUNT") or _creds.get("usrname", "A165741")
PASSWORD = os.environ.get("EECLASS_PASSWORD") or _creds.get("passwd", "")
...
def _click_session_modal(page):
    """If the 'account already logged in elsewhere' modal is up, keep current login (保持登入).
    Do NOT click 登出所有裝置 — that would log out the instructor's other active sessions."""
    # Primary: keep existing sessions (保持登入 / .keepLoginBtn)
    for sel in [
        '.keepLoginBtn',
        'a:has-text("保持登入")',
        'button:has-text("保持登入")',
    ]:
        try:
            btn = page.locator(sel).first
            btn.wait_for(state="visible", timeout=2000)
            print(f"  Handling session conflict modal via keepLogin: {sel}")
            btn.click()
            page.wait_for_timeout(4000)  # wait for navigation (href="####" anchor)
            return True
        except Exception:
            pass
    return False
...
def login(page):
    ...
    Proven login sequence (discovered empirically):
      1. Navigate to /index/login
      2. Fill account + password fields
      3. Click button[data-role='form-submit']
      4. Wait 3s for modal
      5. Click .keepLoginBtn (保持登入) — do NOT logout other sessions
      6. Wait 4s for navigation to complete (anchor href="####" causes hash change)
    """
    print(f"Navigating to /index/login as {ACCOUNT}...")
    page.goto(f"{EECLASS_BASE}/index/login", timeout=15000)
    page.wait_for_load_state("domcontentloaded", timeout=10000)

    page.fill('input[name="account"]', ACCOUNT)
    page.fill('input[name="password"]', PASSWORD)
    page.locator('button[data-role="form-submit"]').click()
    print("  Clicked login button, waiting for modal...")
    page.wait_for_timeout(3000)

    # Dismiss session conflict with 保持登入 (keep login, don't logout other sessions)
